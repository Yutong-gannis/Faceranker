# -*- coding: utf-8 -*-
"""
Created on Thu Aug 18 19:10:15 2022
...
def process_frame(img, net):
    ...
    lw = 2
    im1 = img.copy()
    im1 = cv2.resize(img, (1280, 960))  # BGR通道
    h, w = img.shape[0], img.shape[1]
    blue = (225, 238, 160)
    white = (255, 255, 255)
    gray = (132, 133, 135)
    green = (0, 255, 127)
    img_RGB = cv2.cvtColor(im1, cv2.COLOR_BGR2RGB)  # 转RGB通道
    ...
    if results.multi_face_landmarks:
        ...
        for face_landmarks in results.multi_face_landmarks:
            # 五眼指标
            img, Five_Eye_Metrics, Five_Eye_Diff, FL_X, FL_Y, FT_X, FT_Y, FB_X, FB_Y, FR_X, FR_Y, ELL_X, ELL_Y, ELR_X, ELR_Y, ERL_X, ERL_Y, ERR_X, ERR_Y, Left_Right = WuYan_index(
                img, face_landmarks, h, w)
            # 三庭
            img, Three_Section_Mrtric_A, Three_Section_Mrtric_B, Three_Section_Mrtric_C = SanTing_index(img,
                                                                                                        face_landmarks,
                                                                                                        FT_Y, FB_Y,
                                                                                                        FL_X, FR_X, h,
                                                                                                        w)
            # 达芬奇
            img, Da_Vinci = DaVinci(img, face_landmarks, ELR_X, ELR_Y, ERL_X, ERL_Y, ELL_X, ELL_Y, ERR_X, ERR_Y,
                                    Left_Right, h, w)
            # ResNet预测
            ...
            img = cv2.putText(img, str(i), (FL_X, FT_Y - 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                              green, 2)
            img = cv2.putText(img, 'ResNet-18 Score{:' '>7.1f}'.format(score), (FL_X, FT_Y - 70),
                              cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                              green, 2)
            img = cv2.putText(img, 'Five Eye Metrics{:' '>7.2f}'.format(Five_Eye_Metrics), (FL_X, FT_Y - 50),
                              cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                              green, 2)
            '''
            ...
            img = cv2.putText(img, 'C{:' '>7.2f}'.format(Five_Eye_Diff[4]), (FL_X, FT_Y-75), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                             green, 2)
            '''
            img = cv2.putText(img, 'Three Scetion{:' '>7.2f}'.format(Three_Section_Mrtric_A), (FL_X, FT_Y - 30),
            ...
            img = cv2.putText(img, '1/2{:' '>7.2f}'.format(Three_Section_Mrtric_C), (FL_X, FT_Y-25), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                             green, 2)
            '''
            img = cv2.putText(img, 'Da Vinci{:' '>7.2f}'.format(Da_Vinci), (FL_X, FT_Y - 10), cv2.FONT_HERSHEY_SIMPLEX,
                              0.5,
                              green, 2)
            i = i + 1
...
import time
for i in range(24):
    frame = cv2.imread("D:/face_score/sources/{}.PNG".format(i))
    frame=process_frame(frame, net)
    #展示处理后的三通道图像
    cv2.imshow('my_window',frame)
    time.sleep(0.5)
    cv2.imwrite("D:/face_score/sources/{}(1).jpg".format(i), frame)
